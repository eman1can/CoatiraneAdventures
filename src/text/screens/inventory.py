...
def inventory_battle(console):
    ...
    page_num = int(console.get_current_screen()[len('inventory_battle'):-len('page')])
    ...
    if pickaxe is None:
        ...
        _options[str(option_index)] = f'inventory_battle_select_pickaxe0page/none'
    else:
        display_text += f'\n\n\t{OPT_C}{option_index}:{END_OPT_C} Current Pickaxe: {pickaxe.get_name()}'
        _options[str(option_index)] = f'inventory_battle_select_pickaxe0page/{pickaxe.get_full_id()}'
    option_index += 1

    if shovel is None:
        display_text += f'\n\t{OPT_C}{option_index}:{END_OPT_C} Current Shovel: None'
        _options[str(option_index)] = f'inventory_battle_select_shovel0page/none'
    else:
        display_text += f'\n\t{OPT_C}{option_index}:{END_OPT_C} Current Shovel: {shovel.get_name()}'
        _options[str(option_index)] = f'inventory_battle_select_shovel0page/{shovel.get_full_id()}'
    option_index += 1
    if harvesting_knife is None:
        display_text += f'\n\t{OPT_C}{option_index}:{END_OPT_C} Current Harvesting Knife: None'
        _options[str(option_index)] = f'inventory_battle_select_harvesting_knife0page/none'
    else:
        display_text += f'\n\t{OPT_C}{option_index}:{END_OPT_C} Current Harvesting Knife: {harvesting_knife.get_name()}'
        _options[str(option_index)] = f'inventory_battle_select_harvesting_knife0page/{harvesting_knife.get_full_id()}'
...
def inventory_battle_select(console):
    display_text = get_town_header()
    page_name, current_item_id = console.get_current_screen().split('/')
    page_data = page_name[len('inventory_battle_select_'):]

    page_key = None
    item_list = None
    inventory = Refs.gc.get_inventory()
    for key in ['shovel', 'pickaxe', 'harvesting_knife']:
        if page_data.startswith(key):
            page_key = key
    ...
    if current_item_id != 'none':
        for index in range(len(item_list)):
            if item_list[index].get_full_id() == current_item_id:
                current_item = item_list.pop(index)
                break

    page_num = int(page_name[len('inventory_battle_select_' + page_key):-len('page')])
    ...
    if current_item is not None:
        ...
        _options['1'] = f'inventory_battle_set{page_num}{page_key}/none'
        option_index += 1

    ip_text, ip_options = item_page_list(option_index, f'inventory_battle_set{page_num}{page_key}', page_num, item_list, fail, '', get_inventory_select_item)
