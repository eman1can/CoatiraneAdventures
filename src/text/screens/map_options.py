...
def map_options(console):
    ...
    display_text += '\n\tMap Options\n'
    display_text += f'\n\t{OPT_C}1:{END_OPT_C} Path to Destination'
    display_text += ' - ON' if path else ' - OFF'
    display_text += f'\n\t{OPT_C}2:{END_OPT_C} Current Destination - '
    ...
    if len(layers) == 0:
        ...
        for index, layer in enumerate(layers):
            ...
            _options[str(index + 3)] = f'map_options_toggle_{layer}_{not active}'

    display_text += f'\n\n\t{OPT_C}0:{END_OPT_C} Back\n'
    _options['0'] = 'back'
    _options['1'] = f'map_options_toggle_path_{not path}'
    _options['2'] = 'map_options_change_destination'
