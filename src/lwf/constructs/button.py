...
from ..format.button_condition import KEY_PRESS, PRESS, RELEASE, ROLL_OUT, ROLL_OVER
from ..format.object_data import BUTTON
...
class Button(IObject):
    def __init__(self, *args):
        ...
        if len(args) > 0:
            ...
            super().__init__(l, p, BUTTON, obj_id, inst_id)
    ...
    def roll_over(self):
        ...
        self.play_animation(ROLL_OVER)

    def roll_out(self):
        if not self._handler.empty():
            self._handler.call(ButtonEventHandlers.ROLL_OUT, self)

        self.play_animation(ROLL_OUT)
    ...
    def release(self):
        ...
        self.play_animation(RELEASE)

    def key_press(self, code):
        if not self._handler.empty():
            self._handler.call_key_press(self, code)

        self.play_animation(KEY_PRESS, code)
    ...
    def play_animation(self, condition, code=0):
        ...
        for i in range(0, self.data.conditions):
            ...
            if (c.condition & condition) != 0 and (condition != KEY_PRESS or c.key_code == code):
