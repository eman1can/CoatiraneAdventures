...
from lwf.constructs.animation import CALL, END, EVENT, GOTO_FRAME, GOTO_LABEL, INSTANCE_TARGET_PARENT, INSTANCE_TARGET_ROOT, NEXT_FRAME, PLAY, PREV_FRAME, SET_TARGET, STOP
from lwf.constructs.button_event_handlers import ButtonEventHandlers
from lwf.constructs.movie import Movie
from lwf.constructs.movie_event_handlers import MovieEventHandlers
from lwf.constructs.property import Property
from lwf.modules.color_transform import ColorTransform
from lwf.modules.constant import BLEND_MODE_NORMAL
...
class LWF:
    ...
    def __init__(self, data, renderer_factory, cutom_name):
        self.data = data
        self.renderer_factory = renderer_factory
        self.custom_name = cutom_name
        ...
        self.name = self.data.strings[self.data.name_string_id]
        self.attach_name = None
        self.visible = True

        self.frame_rate = self.data.frame_rate
        self.exec_limit = 3
    ...
    def init(self):
        ...
        self.root_movie = Movie(self, None, self.data.root_movie_id, self.search_instance_id(self._root_movie_string_id))
    ...
    def exec_internal(self, t):
        ...
        if not self._fast_forward:
            if execed or self.property.is_dirty or self._needs_update_for_attach_lwf:
        ...
        while True:
            ...
            if case == SET_TARGET:
                ...
                for j in range(0, count):
                    ...
                    i += 1
                    if inst_id == INSTANCE_TARGET_ROOT:
                        target = self.root_movie
                        continue
                    if inst_id == INSTANCE_TARGET_PARENT:
            ...
            if case == EVENT:
                ...
                if self._event_handlers[event_id] is not None:
                    for eid, h in self._event_handlers[event_id]:
                        h(movie, button)
                continue
            if case == CALL:
                i += 1
