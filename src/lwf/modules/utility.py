...
from lwf.modules.constant import COLOR_TRANSFORM_FLAG, MATRIX_FLAG, MATRIX_FLAG_MASK
...
class Utility:
    ...
    def sync_color_transform(movie):
        ...
        if (color_transform_id & COLOR_TRANSFORM_FLAG) == 0:
            ...
            color_transform_id &= ~-COLOR_TRANSFORM_FLAG
    ...
    def calc_matrix(*args):
        if isinstance(args[0], Matrix):
            ...
            if src_1_id == 0:
                ...
                matrix_id = src_1_id & ~-MATRIX_FLAG_MASK
    ...
    def calc_color_transform(*args):
        if isinstance(args[0], ColorTransform):
            dst, src_0, src_1 = args
            for x in range(4):
                dst.c[x] = src_0.c[x] * src_1.c[x]
            for x in range(4, 8):
                dst.c[x] = src_0.c[x] * src_1.c[x - 4] + src_1.c[x]
            return dst
        else:
            lwf, dst, src_0, src_1_Id = args
            if src_1_Id == 0:
                dst.set(*src_0.c)
            elif (src_1_Id & COLOR_TRANSFORM_FLAG) == 0:
                alpha_transform = lwf.data.alpha_transforms[src_1_Id]
                mr = src_0.c[0]
                mg = src_0.c[1]
                mb = src_0.c[2]
                ma = src_0.c[3] * alpha_transform.alpha
                dst.set(mr, mg, mb, ma, *src_0.c[4:])
            else:
                color_transform_id = src_1_Id & (COLOR_TRANSFORM_FLAG - 1)
