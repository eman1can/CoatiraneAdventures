...
class Timeline(object):
    ...
    def apply(skeleton, lastTime, time, alpha):
        pass
...
class TranslateTimeline(CurveTimeline):
    ...
    def apply(self, skeleton, time, alpha):
        ...
        bone.x = bone.x + (bone.data.x + lastFrameX + (
                    self.frames[frameIndex + self.FRAME_X] - lastFrameX) * percent - bone.x) * alpha
        bone.y = bone.y + (bone.data.y + lastFrameY + (
                    self.frames[frameIndex + self.FRAME_Y] - lastFrameY) * percent - bone.y) * alpha
        return
...
class ScaleTimeline(TranslateTimeline):
    ...
    def apply(self, skeleton, time, alpha):
        ...
        bone.scaleX += (bone.data.scaleX - 1 + lastFrameX + (
                    self.frames[frameIndex + self.FRAME_X] - lastFrameX) * percent - bone.scaleX) * alpha
        bone.scaleY += (bone.data.scaleY - 1 + lastFrameY + (
                    self.frames[frameIndex + self.FRAME_Y] - lastFrameY) * percent - bone.scaleY) * alpha
        return


class FlipXTimeline(Timeline):
    def __init__(self, keyframeCount):
        super(FlipXTimeline, self).__init__(keyframeCount)
        self.LAST_FRAME_TIME = -2
        self.FRAME_SPACING = -self.LAST_FRAME_TIME
        self.FRAME_FLIP = 1
    ...
    def setKeyframe(self, keyframeIndex, time, flip):
        keyframeIndex = keyframeIndex * self.FRAME_SPACING
        self.frames[keyframeIndex] = time
        if flip:
            self.frames[keyframeIndex + 1] = 1
        else:
            self.frames[keyframIndex + 1] = 0

    def apply(self, skeleton, time, alpha):
        if time < self.frames[0]:
            pass
        elif time > self.frames[len(self.frames)-1]:
            pass
        bone = skeleton.bones[self.boneIndex]
        if time >= self.frames[self.LAST_FRAME_TIME]:
            bone.flipX = (bone.data.flipX - 1 + lastFrameX + (
                        self.frames[frameIndex + self.FRAME_X] - lastFrameX) * percent - bone.scaleX) * alpha
