...
def load_equipment_chunk(chunk, loader, program_type, callbacks):
    ...
    if equipment_type == EQUIPMENT_TOOL:
        ...
        equipment = ToolClass(equipment_id, name, tool_type, description, 1.0)
        ...
        equipment = WeaponClass(equipment_id, name, weapon_type, description, weight_values)
        ...
        for weight in weights.split(', '):
            weight_values.append(float(weight))
        equipment = ArmorClass(equipment_id, name, armor_type, description, weight_values)
