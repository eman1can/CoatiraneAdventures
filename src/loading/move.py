...
def load_move_chunk(line, loader, program_type, callbacks):
    ...
    for index in range(effect_count):
        effect_type = int(values[effect_index + 1])
        ...
        if effect_type == STAT:
            effect_sub_type = int(values[effect_index + 2])
            effect_target = int(values[effect_index + 3])
            effect_amount = int(values[effect_index + 4])
            effect_duration = int(values[effect_index + 5])
